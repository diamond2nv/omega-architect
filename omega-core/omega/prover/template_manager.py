...
# ── Optional IEEE import ──────────────────────────────────

_HAS_OMEGA_LLM = False
try:
    from omega.llm import make_langchain_generate_fn  # pyright: ignore[reportMissingImports]

    _HAS_OMEGA_LLM = True
except ImportError:
    make_langchain_generate_fn = None  # type: ignore[assignment]
...
def _build_generate_fn(model: str, base_url: str | None = None) -> Any | None:
    ...
    if not _HAS_OMEGA_LLM or make_langchain_generate_fn is None:
        logger.warning("omega.llm not available — install omega or omega-core[llm]")
        return None
